...
Constituição dessas ninguém brinca se não quer passar mal''\footnote{\emph{Op.
  cit.,} posição 777 (\emph{eBook}).}. Mas ``onde a Constituição
...
efetivos de poder (\emph{Op. cit.}, posição 753 \emph{no eBook}).
\end{quote}

A partir destas considerações Lassalle chega a uma importante conclusão.
Para ele, ``os problemas constitucionais não são problemas de
...
previstas na Constituição'', suspender ``os direitos politicos de
